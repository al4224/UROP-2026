...
# langdetect randomises; pin it so a rerun of the same batch classifies alike.
DetectorFactory.seed = 0
# Calibrated on 643 real abstracts: the two non-English ones scored 0.000 and
# every English one scored 0.286 or better, including ESL-authored writing.
MIN_ENGLISH_PROB = 0.10
...
def rebuild_abstract(inv):
    ...
    return close_split_hyphens(" ".join(tok for _, tok in words))


def close_split_hyphens(text):
    # Tokenisation splits hyphenated terms, so rejoining leaves "continuous-
    # flow" and "N -aryl"; close the gap so quoting the text works.
...
def is_english(text):
    try:
        english = [lang.prob for lang in detect_langs(text) if lang.lang == "en"]
    except LangDetectException:
        return True          # too few real words to judge; the length rule catches these
    return max(english or [0.0]) >= MIN_ENGLISH_PROB
